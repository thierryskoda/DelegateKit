...
function isMeaningfulOutlookCalendarChange(payload: Record<string, unknown>): boolean {
...
export async function recordOutlookCalendarEventChangeAndMaybeEnqueueWorkItem(
  ...
  if (!isMeaningfulOutlookCalendarChange(input.payload)) {
    return {
      enqueuedWorkItem: false,
      joinedExistingWorkItem: false,
    };
  }
  const eventType = "outlook_calendar.event.changed";
