...
import {
  ...
  markMicrosoftOnedriveSubscriptionUnhealthy,
  maybeMicrosoftOnedriveConnectionBySubscription,
  microsoftOnedriveProviderState,
  requireMicrosoftOnedriveConnectionBySubscription,
} from "./connection";
...
async function handleLifecycle(input: {
  ...
  const { connection, state } = await requireMicrosoftOnedriveConnectionBySubscription(
    input.db,
    input.subscriptionId,
  );
  await markMicrosoftOnedriveSubscriptionUnhealthy(input.db, {
    subscriptionId: state.id,
    error: `microsoft_onedrive.webhook.${input.lifecycleEvent}`,
    expiresAt: input.expiresAt ?? null,
  });
  await enqueueMicrosoftOnedriveSubscriptionReconcileJob(input.db, {
...
export async function applyMicrosoftOnedriveWebhook(input: {
  db: SupabaseServiceClient;
  body: unknown;
  headers?: Headers;
}): Promise<{
  ...
  for (const notification of parsed.data.value) {
    const subscription = await maybeMicrosoftOnedriveConnectionBySubscription(
      input.db,
    ...
    const providerState = microsoftOnedriveProviderState(state);
    if (notification.clientState !== providerState.clientState) {
      throw new DomainError(
        domainCodes.UNAUTHORIZED,
        "Microsoft OneDrive webhook clientState did not match subscription state.",
      );
    }
