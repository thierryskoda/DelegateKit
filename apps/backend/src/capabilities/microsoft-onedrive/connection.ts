...
export const MICROSOFT_ONEDRIVE_ADAPTER_KEY = "microsoft_onedrive.drive" as const;
export const MICROSOFT_ONEDRIVE_PROVIDER_KEY = "microsoft-onedrive";
export const MICROSOFT_ONEDRIVE_NANGO_PROVIDER_CONFIG_KEY = "ai-assistants-microsoft-onedrive";
export const MICROSOFT_ONEDRIVE_RESOURCE_TYPE = "microsoft_onedrive.drive" as const;

export type MicrosoftOnedriveConnectionContext = {
  profileId: string;
...
export async function requireMicrosoftOnedriveNango(
  ...
  return requireNangoProviderCapabilityAccount(db, {
    ...
    capabilitySlugs: ["microsoft-onedrive"],
    connectedAccountId: connectedAccountId ?? null,
  });
}

export async function requireMicrosoftOnedriveConnectionByConnectedProviderAccountId(
  db: SupabaseServiceClient,
  connectedProviderAccountId: string,
): Promise<MicrosoftOnedriveConnectionContext> {
  ...
  const connectedProviderAccount = requireSupabaseData(
    "Load Microsoft OneDrive provider connection",
    accountResult.data,
    accountResult.error,
  );
  const binding = await requireMicrosoftOnedriveNango(
    db,
...
export async function requireMicrosoftOnedriveConnectionBySubscription(
  db: SupabaseServiceClient,
  externalSubscriptionId: string,
): Promise<{
  connection: MicrosoftOnedriveConnectionContext;
...
export async function maybeMicrosoftOnedriveConnectionBySubscription(
  db: SupabaseServiceClient,
  externalSubscriptionId: string,
): Promise<{
  connection: MicrosoftOnedriveConnectionContext;
...
export function microsoftOnedriveProviderState(subscription: ProviderWebhookSubscription): {
  clientState: string;
  resource: string;
  driveId: string;
...
export function microsoftOnedriveCursor(subscription: ProviderWebhookSubscription): {
...
export async function listMicrosoftOnedriveSubscriptionsForConnectedAccount(
  db: SupabaseServiceClient,
  connectedProviderAccountId: string,
...
export async function markMicrosoftOnedriveSubscriptionUnhealthy(
  db: SupabaseServiceClient,
