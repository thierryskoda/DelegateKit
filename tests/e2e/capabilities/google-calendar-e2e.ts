...
import {
  ...
  type GoogleCalendarToolName,
} from "@ai-assistants/google-calendar-contracts/contracts";
...
const CAPABILITY_ID = "google-calendar";
const CALENDAR_TIME_ZONE = "America/Toronto";
const EVENT_DAYS_FROM_NOW = 10;
const PRIMARY_CALENDAR_ID = "primary";

const googleCalendarCoverage = createCapabilityToolCoverage(
  CAPABILITY_ID,
  googleCalendarToolContracts,
);
...
async function typedGoogleCalendarTool<const T extends GoogleCalendarToolName>(
  db: SupabaseServiceClient,
  toolName: T,
  params: Record<string, unknown>,
  options?: { trusted?: boolean },
) {
  googleCalendarCoverage.exercise(toolName);
  ...
  return parseCapabilityToolOutput(result, googleCalendarToolContracts, toolName);
}
...
async function cancelGoogleCalendarEventIfNeeded(input: {
  db: SupabaseServiceClient;
  ...
  try {
    ...
    const cancelWrite = await typedGoogleCalendarTool(
      input.db,
...
test("Testing client: Google Calendar capability lifecycle works end-to-end.", async (t) => {
  ...
  const marker = createMarker("testing-calendar");
  ...
  try {
    const accounts = await typedGoogleCalendarTool(db, "google_calendar_accounts_list", {});
    assert.ok(
      accounts.accounts.some((account) => account.connectedAccountId === connectedAccountId),
      `calendar_accounts_list must include connected account ${connectedAccountId}`,
    );

    const calendars = await typedGoogleCalendarTool(db, "google_calendar_calendars_list", {
      connectedAccountId,
    });
    assert.equal(calendars.provider, "google-calendar");
    assert.ok(calendars.calendars.length > 0, "calendar_calendars_list must return calendars");

    const createWrite = await typedGoogleCalendarTool(
      db,
    ...
    const eventGet = await typedGoogleCalendarTool(db, "google_calendar_event_get", {
      connectedAccountId,
      calendarId: PRIMARY_CALENDAR_ID,
      eventId,
      timeZone: CALENDAR_TIME_ZONE,
    ...
    const updateWrite = await typedGoogleCalendarTool(
      db,
    ...
    const updatedEvent = await typedGoogleCalendarTool(db, "google_calendar_event_get", {
      connectedAccountId,
      calendarId: PRIMARY_CALENDAR_ID,
      eventId,
      timeZone: CALENDAR_TIME_ZONE,
    ...
    const eventsList = await typedGoogleCalendarTool(db, "google_calendar_events_list", {
      connectedAccountId,
      calendarId: PRIMARY_CALENDAR_ID,
    ...
    assert.ok(
      ...
      `google_calendar_events_list must include created event ${eventId}`,
    );

    const eventsSearch = await typedGoogleCalendarTool(db, "google_calendar_events_search", {
      connectedAccountId,
      calendarId: PRIMARY_CALENDAR_ID,
      query: marker,
      timeMin: eventWindow.timeMin,
    ...
    assert.ok(
      eventsSearch.events.some((event) => event.id === eventId),
      `google_calendar_events_search must find event ${eventId} by marker query`,
    );

    const freebusy = await typedGoogleCalendarTool(db, "google_calendar_freebusy_query", {
      connectedAccountId,
    ...
    assert.equal(freebusy.provider, "google-calendar");
    assert.ok(Array.isArray(freebusy.busy));

    const freeSlots = await typedGoogleCalendarTool(db, "google_calendar_free_slots_find", {
      connectedAccountId,
      calendarIds: [PRIMARY_CALENDAR_ID],
      timeMin: eventWindow.timeMin,
      timeMax: eventWindow.timeMax,
    ...
    assert.equal(freeSlots.provider, "google-calendar");
    assert.ok(
      freeSlots.freeSlots.some((slot) => slot.durationMinutes >= 30),
      "google_calendar_free_slots_find must return at least one slot of 30 minutes or longer",
    );
    ...
    googleCalendarCoverage.assertComplete();
      ...
      JSON.stringify(
        {
          ...
          connectedAccountId,
          eventId,
          contractTools: googleCalendarToolContracts.map((contract) => contract.name),
        },
        null,
        2,
      ),
    ...
    try {
      eventCancelled = await cancelGoogleCalendarEventIfNeeded({
        db,
        connectedAccountId,
        calendarId: PRIMARY_CALENDAR_ID,
        eventId,
